...
cd /var/www/html/magento2
sudo php bin/magento setup:install \ --base-url=http://192.168.0.41 \ --db-host=localhost \ --db-name=magento2 \ --db-user=magento2 \ --db-password=admin1234 \ --admin-firstname=Admin \ --admin-lastname=Admin \ --admin-email=admin@admin.com \ --admin-user=admin \ --admin-password=admin1234 \ --language=en_US \ --currency=USD \ --timezone=America/Chicago \ --use-rewrites=1 \ --search-engine=elasticsearch7 \ --elasticsearch-host=localhost \ --elasticsearch-port=8080 
cd 
...
sudo systemctl restart apache2
sudo mysql -u magento2 -p'admin1234' -e "use magento2; UPDATE core_config_data SET value='http://192.168.0.41' WHERE path='web/unsecure/base_url';"
cd /var/www/html/magento2/
